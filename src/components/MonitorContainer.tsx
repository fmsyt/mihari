...
const MonitorContainer = () => {
  ...
  useEffect(() => {
    ...
    const fn = async () => {
      const chartList: ChartProviderProps[] = Object.keys(resource).reduce(
        (prev, key) => {
      ...
      unlisten = await listen<UpdateResourceEventPayload>(
        "resourceUpdated",
        ({ payload }) => {
          setChartList((prev) => {
  ...
  return (
    ...
    >
      ...
      {chartList.map((props) => (
        <ChartProvider key={props.id} {...props}>
          <Grid item>
            <Panel width="3em">
